...
└────────────────────────────┼────────────────────────────────────┘
                             │
            1. Direct        │ Authorization header    [default]
               OR            │
            2. Request Token │ (device_id, api_key)   [--token-service]
                             ▼
┌─────────────────────────────────────────────────────────────────┐
...
# Device authentication (token service mode only)
...
```bash
cd device-simulator
python3.12 -m src.main --token-service
```

Speak into your microphone and watch real-time transcription. Press **Ctrl+C** to stop.
...
python3.12 -m src.main --diarization

# Map speakers to real names
python3.12 -m src.main --diarization --speakers Alice Bob
```
...
```bash
python3.12 -m src.main --keyterms "iTranslate" "AssemblyAI" "Pocketalk"
```
...
```bash
python3.12 -m src.main --output ./transcripts/session.txt

# Combined with diarization
python3.12 -m src.main --diarization --speakers Alice Bob --output ./transcripts/session.txt
```
...
python3.12 -m src.main --min-turn-silence 50 --max-turn-silence 500

# More patient — waits longer before finalizing
python3.12 -m src.main --min-turn-silence 200 --max-turn-silence 2000
```

### Session Duration (Token Service Mode)

Controls how long the streaming session can run. Only applies when using `--token-service`.

```bash
# 3 hour session
python3.12 -m src.main --token-service --session-duration 10800

# 10 minute session
python3.12 -m src.main --token-service --session-duration 600
```
...
curl -X POST http://localhost:3001/api/token \
  ...
  -d '{"expires_in_seconds": 300, "max_session_duration_seconds": 3600}'
```
...
`keyterms_prompt` improves recognition of product names, proper nouns, and domain-specific terminology. Pass terms via `--keyterms`. Cannot be combined with a free-text prompt — keyterms only.
...
| `DEVICE_API_KEY` | Device's API key for token service auth | Required for `--token-service` |
...
- Run without `--token-service` for direct mode
...
**Process won't exit on Ctrl+C**
- The process will exit within 4 seconds after Ctrl+C — it finalizes the current turn before closing
- If stuck, run: `pkill -f "src.main"`
